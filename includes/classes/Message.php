...
class Message {
    ...
    public function getConvos(){
        ...
        foreach ($convos as $username) {
            ...
            $return_string .= "<a  role='listitem' title='Click this Message from ".$user_found_obj->getFirstAndLastName()." to message them back' href='messages.php?u=$username'> 
    ...
    public function getConvosDropdown($data,$limit){
        ...
        if($count > $limit){
            ...
            $return_string .= "<a role='link' href='messages.php'><div class='dropdown_data_window_footNote'><input type='hidden' class='noMoreDropdownData' value='true'> <p style='text-align: center;'> Nothing more to show!<br> Click to Access Messenger </p></div></a>";
        }
    ...
    public function getUnreadNumber() {
        $userLoggedIn = $this->user_obj->getUsername();
        $result = 0;
        $query = "SELECT COUNT(tab.n) FROM (SELECT user_from AS n FROM messages WHERE opened = 'no' AND user_to = ? GROUP BY user_from) AS tab";
        if($stmt = mysqli_prepare($this->con,$query)){
